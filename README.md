<div align="center">

# Pedro Henrique Sanches Leal

**Desenvolvedor Full-Stack** · Arquitetura de software · Sistemas inteligentes

```text
"Arquitetura é a arte de fazer decisões que importam."
```

</div>

---

<div align="center">

| [Sobre](#sobre) | [Stack](#stack) | [Projetos](#projetos) | [Contato](#contato) |
|:---:|:---:|:---:|:---:|

</div>

---

## Sobre

Engenheiro de software em formação, focado em construir sistemas **performáticos, seguros e bem arquitetados**. Trabalho com desenvolvimento full-stack, desde a modelagem de dados até interfaces cinematográficas — sem entregar código genérico.

Atualmente desenvolvendo **Izanagi AI**, um framework de engenharia de software autônoma orientada a agentes (CLI publicada no npm), com arquitetura em camadas: Decision → Context → Skill → Quality → Reflection → Memory.

## Stack

| Categoria | Tecnologias |
|:---|:---|
| **Linguagens** | ![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white) ![C#](https://img.shields.io/badge/C%23-239120?style=flat-square&logo=csharp&logoColor=white) ![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white) ![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white) ![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white) |
| **Back-end** | Node.js · .NET 8 · APIs REST |
| **Front-end** | Next.js · Tailwind CSS · Web Animations (GSAP, Three.js/WebGL) |
| **Bancos de dados** | PostgreSQL · Redis |
| **DevOps** | Docker · GitHub Actions · CI/CD · Terraform |
| **IA / Agentic** | LLMs · RAG · LangGraph · MCP · Multi-agent systems |
| **Arquitetura** | Clean Architecture · DDD · CQRS · Design Patterns |

## Projetos

<div align="center">

### Izanagi AI
*Framework meta para engenharia de software autônoma — 12 agentes, 79+ skills, engines de decisão e memória, CLI npm.*

[![npm](https://img.shields.io/badge/izanagi--ai-09090b?style=flat-square&logo=npm&logoColor=white)](https://www.npmjs.com/package/izanagi-ai)
[![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)](https://github.com/pedrohenriquesanchesleal4-debug/izanagi-ai)

### Site Izanagi
*Site do framework — estética Apple-like, glassmorphism, scrollytelling e micro-interações.*

[![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)](https://github.com/pedrohenriquesanchesleal4-debug/SiteIzanagi)

### Sistema de Cadastro de Livros
*.NET 8 — CRUD completo de cadastro, exclusão e listagem de livros.*

[![C#](https://img.shields.io/badge/C%23-239120?style=flat-square&logo=csharp&logoColor=white)](https://github.com/pedrohenriquesanchesleal4-debug/Sistema-de-Cadastro-de-Livros)

</div>

<details>
<summary><b>📦 Mais repositórios</b></summary>

- **Calculadora-Simples** — Calculadora com operações básicas e decimais em C#/.NET 8
- **Verificador-de-numeros-impares-ou-pares** — Verificador par/ímpar em C#/.NET 8
- **Portfólio-v1** — Meu primeiro portfólio

</details>

## Estatísticas

<div align="center">

[![GitHub Stats](https://github-readme-stats-six-eta.vercel.app/api?username=pedrohenriquesanchesleal4-debug&show_icons=true&theme=radical&hide_border=true&bg_color=0d1117&title_color=58a6ff&icon_color=58a6ff&text_color=c9d1d9)](https://github.com/anuraghazra/github-readme-stats)
[![Top Langs](https://github-readme-stats-six-eta.vercel.app/api/top-langs/?username=pedrohenriquesanchesleal4-debug&layout=compact&theme=radical&hide_border=true&bg_color=0d1117&title_color=58a6ff&text_color=c9d1d9)](https://github.com/anuraghazra/github-readme-stats)

[![GitHub Streak](https://streak-stats.demolab.com/?user=pedrohenriquesanchesleal4-debug&theme=radical&hide_border=true&background=0d1117&stroke=58a6ff&ring=58a6ff&fire=58a6ff&currStreakNum=c9d1d9&sideNums=c9d1d9&currStreakLabel=58a6ff&sideLabels=8b949e&dates=8b949e)](https://git.io/streak-stats)

</div>

## Contato

<div align="center">

[![Email](https://img.shields.io/badge/Email-09090b?style=for-the-badge&logo=gmail&logoColor=white)](mailto:pedrohenriquesanchesleal4@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/pedro-henrique-sanches-leal-a7a721278)
[![Portfólio](https://img.shields.io/badge/Portfólio-09090b?style=for-the-badge&logo=vercel&logoColor=white)](https://pedrohsl-portfolio.vercel.app)

</div>

---

<div align="center">

*Construído com disciplina, arquitetura e alto padrão de qualidade.* ⬢

</div>
